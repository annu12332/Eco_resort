    ...
    return (
        ...
        <div className="min-h-screen bg-stone-50 flex font-sans relative overflow-x-hidden">
            ...
            <div className="flex-1 flex flex-col min-h-screen w-full md:ml-64 transition-all duration-300">
                ...
                {/* Header */}
                <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-stone-100 px-4 md:px-12 py-4 md:py-5 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => setIsSidebarOpen(true)}
                            className="p-2.5 bg-stone-100 rounded-xl md:hidden text-stone-600 hover:bg-stone-200 transition-colors"
                        >
                            <Menu size={22} />
                        </button>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] text-emerald-700 font-black uppercase tracking-[0.2em] mb-1">Eco Resort</p>
                                <motion.h1
                                    key={activeTab}
                                    initial={{ opacity: 0, y: -5 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="text-xl md:text-2xl font-black text-stone-900 tracking-tight"
                                >
                                    {tabTitles[activeTab]}
                                </motion.h1>
                            </div>
                    ...
                    <div className="flex items-center gap-3 md:gap-6">
                        <div className="relative p-2 text-stone-400 hover:text-emerald-600 cursor-pointer transition-colors hidden sm:block">
                            <Bell size={22} />
                        ...
                        <div className="flex items-center gap-3 pl-4 border-l border-stone-100">
                            <div className="text-right hidden sm:block">
                                <p className="text-sm font-bold text-stone-900 leading-none">Admin Manager</p>
                            ...
                            <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-700 rounded-2xl shadow-lg shadow-emerald-200 flex items-center justify-center text-white font-black text-xl">
                                A
                            </div>
                        </div>
                    </div>
                                ...
                                {activeTab === 'stats' && (
                                    <div className="space-y-8">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
                                            <StatsCard label="Total Cabins" value={stats.totalRooms} icon={<Trees size={22}/>} color="emerald" />
                                            <StatsCard label="Active Packages" value={stats.totalPackages} icon={<Package size={22}/>} color="emerald" />
                                        ...
                                        {/* System Overview updated to Emerald */}
                                        <div className="relative overflow-hidden bg-emerald-950 rounded-[2rem] p-8 md:p-12 text-white shadow-2xl">
                                            ...
                                            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                                                <div>
                                                    ...
                                                    <p className="text-emerald-200 font-medium max-w-md">
                                                ...
                                                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                                                    <button
                                                        ...
                                                        className="bg-emerald-500 text-white px-8 py-4 rounded-full font-bold hover:bg-emerald-600 transition-all shadow-xl active:scale-95"
                                                    >
                                                        Review Bookings
                                                    </button>
                                                    <button
                                                        onClick={() => setActiveTab('manage-packages')}
                                                        className="bg-emerald-900 border border-emerald-800 text-white px-8 py-4 rounded-full font-bold hover:bg-emerald-800 transition-all active:scale-95 flex items-center justify-center gap-2"
                                                    >
...
const StatsCard = ({ label, value, icon, color }) => {
    // StatsCard color palette updated
    const colors = {
    ...
    return (
        <motion.div whileHover={{ y: -5 }} className="bg-white p-6 rounded-[2rem] shadow-sm border border-stone-100 flex flex-col gap-4">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${colors[color]}`}>
                {icon}
            </div>
            <div>
                <p className="text-stone-400 font-bold text-[11px] uppercase tracking-widest">{label}</p>
                <h3 className="text-2xl md:text-3xl font-black text-stone-900 mt-1 tracking-tight">{value}</h3>
            </div>
        </motion.div>
    );
};
